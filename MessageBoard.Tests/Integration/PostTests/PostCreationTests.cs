...
public class PostCreationTests : IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task PostCanBeCreatedForTopic()
    {
    ...
    [Fact]
    public async Task PostCanBeCreatedForPrivateMessage()
    {
        using var scope = _factory.Services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<MessageBoardDbContext>();
        dbContext.Database.EnsureDeleted();
    ...
    public async Task PostCannotBeCreatedInNonExistentTopic()
    {
        ...
        var response = await _client.PostAsync("/discussions/1/posts", content);
